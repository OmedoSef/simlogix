...
Wires are currently a straight line pin-to-pin; custom routing (waypoints, orthogonal routing) is planned but not built.

Not implemented yet: wire routing (waypoints/orthogonal), logic gates, sub-circuit hierarchy (multiple circuits per project, used as components inside each other).
