...
- **`Level`** — what one bit is doing: `High`, `Low`, `Unknown`, `Error`, or `HighZ`, plus the two weak levels a pass transistor delivers. Unknown/error states are modeled from the start, since mishandling X/Z-like states is a common source of subtle simulation bugs. `HighZ` is distinct from `Unknown`: `Unknown` means "not driven / not simulated yet", `HighZ` means "this driver is deliberately not driving right now" (tri-state), needed for bidirectional buffers.
- **`Signal`** — what a *wire* carries: one `Level` per bit, **least significant first**. A plain wire is a signal of width one, which is what every wire is until the drawing says otherwise, so there is no scalar case and vector case anywhere in the engine — one shape, with the truth tables underneath still working on a single level at a time.

  `Signal::only_level` answers the level of a one-bit signal and `Error` for any other width: a component with no meaning on a bus says so on the wire rather than quietly reporting bit 0. `component::scalar_eval` wraps a component whose body is written in levels, which every one of them is today; the name is the honest one, and it stops being true one component at a time as each learns what a bus means for it.
...
- **`NetGroup`** — one net the drawing says exists: the pins it joins **and how many bits it carries**. Both are derived from the drawing and handed to `rewire` together, because they answer the same question — what does this conductor touch. Width could have lived on the `Pin` instead; it didn't, because a pin's width is only ever what its component's properties say, and whoever reads those already owns the drawing.
- **`Circuit`** — the graph of components and nets, plus the event queue and logical clock. A `Net` can have more than one potential driver (to support tri-state buffers); resolution runs **bit by bit**, so the meaning never depends on the width: per bit, ignore `HighZ` drivers, then 0 remaining → `Unknown`, 1 → that value, ≥2 differing → `Error`.

  A net's width is the one the drawing declared, so an *undriven* net still reads that many unknown bits rather than one — a component reading an empty bus sees a bus. A contribution of any other width faults **every** bit: padding or truncating it would be inventing bits nobody asked for, where `Error` is exactly the claim to make and is visible on the wire.
